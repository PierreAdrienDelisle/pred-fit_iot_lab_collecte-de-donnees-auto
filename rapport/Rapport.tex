...
\subsection{Contiki}
Contiki est un système d'exploitation open-source, designé pour l'IoT, particulièrement pour la mise en place de protocoles réseaux (TCP/IP, IPv6, Low-Power protocols), basé sur le système de proto-threads.
...
Par ailleurs, des exemples permettent aussi de comprendre le fonctionnement global de la plateforme et d'en extraire les principes de base. Malgré cela, il n'existe pas d'exemple traitant directement avec les capteurs, et il nous faudrait donc aller chercher plus en profondeur dans la documentation pour trouver des solutions à ce niveau.

\subsection{Analyse}
Étant donné le peu d'expertise que nous possedons dans le domaine, nous ne pouvons pas donner un avis définitif quant au système d'exploitation à utiliser. En effet  chacune des solutions nous aurait permis à terme d'arriver à une solution très satisfaisante. En effet, le problème de base, au niveau du firmware, est relativement élémentaire et ne demande pas de programmation très compliquée.
...
\section {Automatisation via Linux}
	...
	Systemd et cron sont deux solutions possibles pour notre script python. L'avantage de systemd est le fait qu'il soit plus complet et que plusieurs tâches peuvent être aisément exécutées et débuggées en même temps. Néamoins, il introduit plus de complexité avec un système de fichiers là où crontab ne nécessite qu'une ligne. De plus crontab à un système de mail qui nous peut nous alerter si une tâche ne s'est pas effectué correctement. Ainsi dans notre cas où nous n'avons qu'une tâche simple à effectuer, il semble approprié de déployer crontab plutôt que systemd. Néanmoins, si la solution venait à évoluer par la suite pour avoir plusieurs relevés différents avec d'autres scripts, il serait recommandé d'utiliser systemd. 
	
\section{Protocole RPL}

RPL (Routing protocol for Low-power and Lossy networks)), est un protocole de routage qui est adapté à l'IoT, défini dans la RFC 6550 [11]. Basé sur un système d'arbre de routage, il se base sur deux types de n\oe uds: Dans le réseau, un client est désigné comme étant routeur de bordure. Ce routeur aura pour tâche de renvoyer les paquets remontant vers lui vers la branche qu'il faut. Les autres clients seront alors de simples clients qui renverront leurs paquets vers leur père dans la hiérarchie de l'arbre.
...
En termes de réseau, il nous fallait une métrique qui nous permettrait d'évaluer la qualité du réseau, ainsi que le bruit ambiant. Nous avons découvert trois métriques différentes, qui auraient pu être acceptables dans notre sujet et qui permettent de faire ces deux choses. Il existe probablement d'autres indicateurs permettant de récupérer ces informations, mais nous avons préféré nous concentrer sur les trois suivantes:
...
\begin{itemize}
	...
	\item Data est le dossier contenant les bases de données. Il existe par défaut la base de données _internal contenant un certain nombre d'informations générales sur nos bases de données. Puis ensuite, il existe toutes les données relatives à nos collectes.
...
[Cette partie sera remplie dès que j'aurai pu obtenir assez de résultats pour émettre une conclusion quant à notre travail. Jusqu'ici, et avec les tests que j'ai pu effectuer ce mardi, il semblerait qu'augmenter le nombre de n\oe uds change fortement la reproductibilité de l'expérience. Je suis en collection de données à l'instant même]
