...
La plateforme est décomposée en plusieurs parties :
\begin{itemize}
	\item Site web - FIT IoT Lab
	...
	La frontend SSH est un ordinateur distant auquel chaque utilisateur de FIT IoT Lab peut se connecter via son compte. Elle est utile pour lancer directement des expériences, les gérer et prendre des informations diverses et variées. Cette frontend SSH s'appuie sur l'API REST pour fonctionner mais elle permet d'utiliser des scripts en local sur cet ordinateur distant afin de réaliser des tâches plus complexes comme le "serial aggregator" qui va agréger toutes les entrées des liens séries de nos noeuds. Les scripts sont écrits généralement en Python.
...
\begin{figure}[hp]
    \includegraphics[width=0.5\textwidth]{Images/web-view_archi}
...
Dans cette section, nous présenterons les tutoriels essentiels de la plateforme que nous avons dû suivre pour réaliser notre solution.
	\begin{itemize}
		\item Getting started ? the tutorial for beginners 
		...
		\item Experiment CLI Client
		\item Run a script in background
		\item Nodes Serial Link Aggregation
\end{itemize}	 
...
\newline
Dans le premier benchmarking [1], celui ci pointe plusieurs aspects à savoir :
\begin{itemize}
...
\newline
Pour recentrer les recherches sur la solution à choisir nous avons donc pris en compte le classement de popularité des bases de données fait par db-engines [3]. Celui-ci permet d'avoir une vision sur les bases qui sont très utilisées, mises à jour et donc actives.
\begin{figure}[hp]
...
\begin{figure}[h]
    ...
    \caption{Temps de réponse moyen en fonction de la charge}
\end{figure}
